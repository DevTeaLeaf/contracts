...
contract Lottery is Ownable {
    ...
    address token = 0x3B5328D38a795514E044081fcaa764013715C666;
    ...
    uint lotteryPrice = 10 ether;

    uint commission = 100;
    uint division = 10000;
    uint jackpotPerc = 6000;
    uint deliveryFee = 99 ether;

    uint constant ONE_WEEK = 604800;

    constructor(address _admin) {
        admin = _admin;
        nextLockTime.push(1111111111); //lock
        nextLockTime.push(1111111111); //1 number
        nextLockTime.push(1111111111); //2 number
        nextLockTime.push(1111111111); //3 number
        nextLockTime.push(1111111111); //4 number
    ...
    mapping(uint => uint) private pool; //number of lottery -> prize pool
    ...
    function getWinner(uint[6] memory winNumber) external {
        ...
        IERC20(token).transfer(feeAddr,fee);
    ...
    function _transferPrize(address[] memory winner,uint prize) private {
        for(uint i;i < winner.length;i++) {
            IERC20(token).transfer(winner[i], prize);
        }
    }
    ...
    //========================================Shop============================================= -> below par / optional

    event boughtToy(address who,uint when,uint paid);

    function buyToy(uint amount) external {
        uint toPay = getPrice(deliveryFee);
        IERC20(token).transferFrom(msg.sender, store, (toPay * amount));
